...
try:
    ...
    raise SystemExit("Install the parser first: python3 -m pip install PyYAML==6.0.3") from exc
...
def source_paths(root, excluded):
    ...
    for name in sorted(set(raw.decode().split("\0"))):
        ...
        if not name or not path.is_file() or path.is_symlink():
            continue
        if any(name == item or name.startswith(item.rstrip("/") + "/") for item in excluded):
            continue
